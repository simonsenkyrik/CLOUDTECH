...
function App() {
  ...
  const loadUsers = useCallback(async () => {
    ...
    const { data, error } = await supabase
      .from("profiles")
      .select("id, username, email, plan, role, approved, created_at")
      .order("created_at", { ascending: false });

    if (error) {
      console.error("Načtení uživatelů selhalo:", error.message);
      setAdminUsers([]);
    } else {
      setAdminUsers(data || []);
    }
    ...
    const checkUser = async () => {
      try {
        ...
        let { data: profileData, error: profileError } = await supabase
          .from("profiles")
          .select("username, plan, role, approved")
  ...
  const isUsersSection = isAdmin && activeSection === "users";
  const isPendingUser = !isAdmin && profile?.approved === false;
  ...
  const storageBarPercent = usedStorage > 0 ? Math.max(storagePercent, 0.6) : 0;
  ...
  const handlePlanChange = async (userId, newPlan) => {
    ...
    if (error) {
      setNotice({
        type: "error",
        text: "Tarif se nepodařilo změnit.",
      });
      return;
    }
    ...
    setNotice({
      type: "success",
      text: "Tarif byl změněn.",
    });
  };

  const handleApproveUser = async (userId, selectedPlan) => {
    const { error } = await supabase
      .from("profiles")
      .update({
        plan: selectedPlan,
    ...
    if (error) {
      setNotice({
        type: "error",
        text: "Uživatele se nepodařilo schválit.",
      });
      return;
    }
    ...
    setNotice({
      type: "success",
      text: "Uživatel byl schválen a tarif byl přidělen.",
    });
  };
  ...
  const handleDownload = async (file) => {
    setNotice({
      type: "info",
      text: `Připravuji stažení souboru ${getFileName(file)}…`,
    });
    ...
    setNotice({ type: "success", text: "Soubor byl stažen." });
  };
  ...
  const handleDelete = async (file) => {
    ...
    if (storageError) {
      setNotice({
        type: "error",
        text: `Smazání selhalo: ${storageError.message}`,
      });
      return;
    }
        ...
        <div className="sidebar-top-row">
          <div className="sidebar-logo">
            <a
              ...
              aria-label="Načíst přehled CLOUDTECH"
            >
        ...
        <header className="dashboard-header">
          <div className="dashboard-title-row">
            ...
            <div>
              <div>
                ...
                <p>
                  {isAdmin
                    ? "Zde můžeš spravovat registrované uživatele."
                    : activeSection === "overview"
          ...
          {!isAdmin && (
            <div className="dashboard-header-actions">
              ...
              >
                {uploading ? (
                  <Loader2 className="spin-icon" size={19} />
                ) : (
                  <Upload size={19} />
                )}
          ...
          <section className="files-panel" id="files">
            ...
            {filesLoading ? (
              ...
              <div className="empty-state">
                ...
                <p>
                  Pro nahrání prvních souborů použijte tlačítko „Nahrát soubory“
                  vpravo nahoře.
                </p>
              </div>
            ) : (
                  ...
                  return (
                    <div className="file-table-row" role="row" key={file.id}>
                      <div className="file-name-cell">
                        <div
                          className={`file-type-icon file-type-icon--${category}`}
                        >
